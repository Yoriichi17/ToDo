...
function Home() {
  ...
  return (
    <div>
      <div>
        <h1 id="head">Your To Do List</h1>
      </div>
      <form id="form" onSubmit={editMode ? updateHandler : createHnadler}>
        <input id="input"
          type="text"
          name="title"
          value={title}
        ...
        />
        <button id="addbtn" type="submit"> {editMode ? "Update" : "Add"} </button>
      </form>
      ...
      <div className="container">
        {todos?.map((todo) => (
          <div className={`card ${todo.completed ? "completed" : ""}`} key={todo?._id}>
            ...
            <p className={`checkBox ${todo.completed ? "CBcompleted" : ""}`}></p>
            <h3>{todo.title}</h3>
            </div>
